...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^ { sharedView = [[self alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
        ...
        sharedView.hudRingForegroundColor = [UIColor blackColor];
        sharedView.maskType = SVProgressHUDMaskTypeGradient;
//        sharedView.hudBackgroundColor = [UIColor blackColor];
//        sharedView.hudForegroundColor = [UIColor whiteColor];
    });
    return sharedView;
}
...
+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:SVProgressHUDMaskTypeGradient];
}
...
+ (void)showProgress:(float)progress {
    [[self sharedView] showProgress:progress status:nil maskType:SVProgressHUDMaskTypeGradient];
}

+ (void)showProgress:(float)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:SVProgressHUDMaskTypeGradient];
}
